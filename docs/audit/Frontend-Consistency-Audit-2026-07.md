...
> Этот документ — **аудит «ДО»** и **карта дрейфа** (снапшот на HEAD `601ea9f`). §5 был предложением ремонта волнами — **исполнен 2026-07-04** (4 волны + бонус, 6 коммитов). Итог исполнения и честный счёт закрытого — **§7**. §1–§4/§6 остаются как зафиксированное состояние «ДО».
...
## §5. План ремонта волнами — ✅ ИСПОЛНЕН (4 волны + бонус, 2026-07-04)

> Изначально — предложение (юзер решил объём). **Все 4 волны исполнены локально**, 6 коммитов (не запушены — main закоммитил/деплой отдельно). Порядок — от «молча сломано» к косметике. Оценки грубые (S≈≤0.5д, M≈0.5–1.5д, L≈2–4д на файл-кластер), риск — вероятность регресса при правке. Итог исполнения и честный счёт закрытого — §7.

**Коммиты исполнения:**
| Волна | Коммит | Суть |
|---|---|---|
| В1 | `4610826` | «сломанное молча»: ~230 фантом-Bootstrap-вхождений (docs ~80 + onboarding ~150) → scoped-токены; 27 мёртвых PrimeVue-3 токенов канваса → живые; битые иконки (`pi-pen-to-square`/`pi-arrow-circle-right`/`pi-verified`/`pi-book`/`pi-calculator`); единая фильтр-строка «Обзора» (снят legacy `DashboardToolbar`) + фикс двойного фетча; `EntityAvatar` на карточках задач |
| В2 | `bb5440d` | dark-долг: 265 `.app-dark`-блоков в 27 файлах аудировано, ~119 точек починено (invisible-ink / light-plate / sub-AA muted / static-primary → reactive); −202 строки (лишние dark-ветки сняты в пользу reactive base-токенов) |
| В3 | `5be5bb8` | периферия → канон-шасси: shared `ListToolbar`/`SegmentedControl`/`FilterTriggerButton`/`PaginatorCaption`; каталог/документы/онбординг/автоматизации на тулбар-канон; embedded-варианты (справочники в Настройках) |
| В4 | `b14f71f` | швы: 5 самодельных оверлеев DealPage → `SearchPicker`/`AutoComplete`/`Menu`/`Popover` (−350 строк); деньги — 2 канон-формы (`formatCurrency` в таблицах + compact в KPI/канбане, 6 локальных копий снято); `NotFoundState` на 6 страниц; унифицированные пагинации; confirm-лейблы |
| бонус | `e7d4b64` | человекочитаемые имена вместо FK-ID в лентах сделки и CRM (additive `old_display`/`new_display`, фронт-потребление, degrade→`#id`); 25 тестов |

**Гейты:** сьют PHPUnit 4081→4097/0 · `type-check` 0 · `lint:ds` 0 · ~7 QA-браузерных гейтов (обе темы) с фикс-раундами.

### Волна 1 — «сломанное молча» (S/M) — ✅ ИСПОЛНЕНО (`4610826`)
...
### Волна 2 — «dark-долг» инвертированной шкалы (M) — ✅ ИСПОЛНЕНО (`bb5440d`)
...
### Волна 3 — «поколение периферии» (L) — ✅ ИСПОЛНЕНО (`5be5bb8`)
...
### Волна 4 — «швы» (M) — ✅ ИСПОЛНЕНО (`b14f71f`; бонус имён из лент — `e7d4b64`)
...
## §7. Итог исполнения (2026-07-04) + осталось

> Раздел добавлен после исполнения §5. Счёт честный: считаю закрытым только то, что **фактически тронуто в diff-е волн** (проверено по `git show --stat`), а не «предложено в плане».

### Что закрыто из §3-семейств
- **(а) Фантомные Bootstrap-утилиты** — ✅ **закрыто** (В1). ~230 инертных вхождений в docs+onboarding+automation переведены на scoped-токены / локальный `.w-full` / `<span class="req">` / PrimeVue. Light-тема больше не ломается молча.
- **(е) Мёртвые PrimeVue-3 токены канваса** — ✅ **закрыто** (В1). 27 токенов (`--p-surface-card/-border/-ground/-overlay/-radius/-hover`) → живые; карточный рецепт нод и hover работают.
- **(з) Битые иконки / severity / несущ. токены (часть «сломано»)** — ✅ **в основном закрыто** (В1 иконки + В4 money). Иконки-глифы починены; money-дубли устранены (2 канон-формы, 6 локальных копий снято, «1.5M»↔«1 200 000 ₽» сведены). Хвост: невалидные `severity="warning"` и `--p-*-rgb`/`--p-info-300` — точечные low, часть могла остаться (см. «осталось»).
- **(г) Dark-долг инвертированной шкалы** — 🟡 **закрыто по горячим зонам** (В1+В2). deal-page (7 high), tasks+cabinet, dashboard-overview, канвас — вычищены; ~119 точек в 27 файлах. **НЕ закрыто целиком:** зона `components/crm/entity/*` (H24, ~25 файлов muted `surface-400/500`) в diff волн **не тронута** — muted-фикс сущностей landed на page-уровне (Contact/Company/Deal под `pages/`), но общий entity-компонентный muted-свип остаётся. Оценка: high-ветка dark закрыта, medium-хвост entity — нет.
- **(б) PageHeader→тулбар / (в) фильтры→паттерн Б / (ж) голые таблицы** — ✅ **закрыто по периферии** (В3). Каталог/документы/онбординг/автоматизации подняты на shared `ListToolbar`/`SegmentedControl`/`FilterTriggerButton`/`PaginatorCaption` + карточные подложки + канон-пагинация. Двойная фильтр-строка «Обзора» снята (В1).
- **(д) Самодельные оверлеи DealPage** — 🟡 **закрыто по DealPage** (В4). 5 самоделок → `SearchPicker`/`AutoComplete`/`Menu`/`Popover` (−350 строк). **Эхо не тронуто:** tasks entity-picker, automation `SearchPicker` (3-я копия) остались.
- **(и) Прочее high/med** — 🟡 **частично.** ✅ `EntityAvatar` на карточках задач (В1); `NotFoundState` на 6 страниц (В4); confirm-лейблы (В4). ❌ **`RequisiteCard` per-card ConfirmDialog (H8) — сознательно оставлен** (вложенные оверлеи, закон-коммент — см. «осталось»); ❌ `:draggable="false"` на все диалоги — **не добавлено** (0 в diff); ❌ дубль `SidebarNotifications` — **не устранён**.
- **Бонус вне §5:** человекочитаемые имена вместо FK-ID в лентах сделки и CRM (`e7d4b64`) — additive display-поля + фронт-потребление + 25 тестов.

### Честный счёт high/medium
- **HIGH (24):** реально закрыто **~20 из 24**. Не закрыто/отложено: **H8** (RequisiteCard confirm — сознательно), **H24** (crm/entity muted ~25 файлов — не тронуто на компонентном уровне). H10/H11 (tasks light-plate) и H12 (двойная фильтр-строка) — закрыты. Иконочные/токенные high (H21/H22/H23) закрыты.
- **MEDIUM (83):** закрыто **~55–60**. Основной незакрытый хвост — entity-cards medium (muted-границы/DataTable-подложки полноразмерных табов ContactDealsTab/CompanyDealsTab и т.п.), эхо самодельных дропдаунов в tasks/automation, отдельные `:draggable`/severity-медиумы. Оценка консервативная — по факту тронутых файлов, а не по «намерению волны».
- **LOW (62):** волны закрывали low попутно (money-литералы, empty-состояния, пагинации), но low как класс **не был целью** — большая часть косметики (rem/px мимо `$space`, i18n-хардкоды копи, инлайн-спиннеры вместо Skeleton) остаётся by-design на будущие проходы.

### Осталось / принятые решения
- **CrmFeed log-виджет** — meta.changes-шейп лога отличается от feed-шейпа; человекочитаемые имена туда не заведены. Отдельный заход (не входил в бонус `e7d4b64`, который покрыл feed, не log-виджет).
- **Порог компакт-денег «тыс.»** — канон compact включается на **≥10k units** (осознанно, консистентно с дашбордом). Мелкие суммы показываются полной формой — это решение, не недоработка.
- **`RequisiteCard` grouped-confirm** — локальный confirm оставлен намеренно: вложенные оверлеи внутри карточки реквизита плохо ложатся на глобальный `useConfirm` без регресса; помечено закон-комментарием. H8 остаётся открытым осознанно.
- **`DealsListView` / `Contacts` пагинаторы** — не переведены на shared `PaginatorCaption`: это зоны sales/crm-пилота, уже близки к канону (собственные кастом-пагинаторы). Оставлены, чтобы не лезть в пилот-зоны без нужды.
- **standalone-роуты `products`/`templates`** отсутствуют **by-design** — тулбары этих модулей живут в обеих ветках (standalone + embedded в Настройках); дубля роутов нет намеренно.
- **crm/entity muted-свип (H24) + entity medium-хвост** — не закрыт; кандидат в отдельный «entity dark-хвост»-проход.
- **`:draggable="false"` / дубль `SidebarNotifications` / эхо самодельных дропдаунов (tasks/automation)** — не тронуты; low-приоритет швы на будущее.

### Процессная заметка (для флота)
За трек было **3 инцидента stash/reset общего рабочего дерева** при параллельных FE-волнах. Правило усилено: **параллельные FE-волны — только в раздельных git-worktree** (добавлено в `.claude/AGENTS.md` §5.1).
...
*Аудит-часть (§1–§4, §6, Приложение A) — read-only снапшот «ДО» на HEAD `601ea9f`. §5 — план ремонта, **исполнен 2026-07-04** (6 коммитов `4610826`→`e7d4b64`). §7 — итог исполнения + честный счёт закрытого/оставшегося.*
