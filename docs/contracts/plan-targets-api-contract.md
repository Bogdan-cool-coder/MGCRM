...
Route::prefix('reports')->name('reports.')->group(function () {
    ...
    Route::get('stage-conversions',[ReportsController::class, 'stageConversions'])->name('stage-conversions'); // R5 stage block, standalone (Ф4)
...
> **As-built (Ф3, reviewer 2026-07-03).** Two underspecs the prose above left open, now fixed as-built:
> - **`supports` semantics.** A "support" (`supports` field / SpaceCRM «сопровождение») = a won deal with **`is_primary_deal=false`** — an upsell/follow-on on an existing client, as opposed to first-acquisition new business (`is_primary_deal=true`). This reuses the flag `DealMoveService` already stamps (no new column); it is the closest first-class MGCRM equivalent of SpaceCRM's new-client-vs-existing-client distinction. Points then split: `deal_points = (won_deals − supports) × points_per_won_deal`, `support_points = supports × points_per_support`, `income_points = round(new_income_base_kopecks / income_divisor_kopecks)`.
> - **Standings pool + read-scope.** "In standings" = **active, non-service (`is_service=false`) users holding the Manager or Director role** (resolved via the spatie `role()` query scope on guard `sanctum` — no inline role check). Everyone else who owns a won deal (service/admin accounts with historical ownership) is "out of standings". The whole leaderboard is **visibility-scoped via `VisibilityResolver`**, mirroring `SalesDashboardService::topManagers` — the one existing precedent for how a cross-manager comparison report behaves under the §8.2 "reads are visibility-scoped, not gated" rule (a Department-scoped manager is ranked only against their department; All-scope director/admin see the whole company).
> - **FX.** Money fact = won deals in the queried year, effective-recognition-date `COALESCE(closed_at, signed_at, paid_at, stage_changed_at)` (reused, never re-derived), non-base currencies converted at the **1st-of-January** rate of the queried year (the annual-cell convention, no monthly granularity in this report).
>
> **Known behavioural gap (flagged, not yet fixed):** `leader` (=`rows[0]`) and the sequential `rank` are computed over **all** rows including out-of-standings ones. In `mode=absolute`, a high-points service/admin account therefore becomes `rows[0]` (the hero `leader`) and shifts real managers down a rank slot, while the FE renders that row's rank as «—». `mode=standard` (default) is unaffected. Correct fix = derive `leader` + assign `rank` from in-standings rows only (out-of-standings keep `rank:null`).
...
> **As-built (Ф4, reviewer 2026-07-03):** the `stage` block ships as a **flat period-aggregate**, NOT the per-month `cells`-keyed shape sketched above. Each transition row is `{ name, from_stage_id, to_stage_id, den_count, num_count, fact_pct }` at the top level — one honest first-entry conversion for the whole selected period (`?year`/`?month` optional; no month → all-time). This is by design: honest stage conversion is a *first-entry-only* count over the period (a deal that enters stage[i] once contributes once; returns/branches never double-count — see `StageConversionService` docblock), so there is no meaningful per-month cell breakdown. The standalone `GET /api/reports/stage-conversions` returns `{ meta, chain: [...] }` (same flat rows under `chain`); when embedded in R5 the identical rows appear under `stage`. FE `StageConversionFunnel` renders these as progress bars (not ECharts — an obvious progress primitive) and defensively skips malformed rows (empty funnel, never a crash). `custom` pairs keep the per-month `cells` shape as documented.
...
**Ф3 — R3 Best manager. ✅ DONE (uncommitted, 2026-07-03, QA e2e PASS, 3896 PHPUnit).** `BestManagerService` (yearly points from `config('crm.plans.best_manager')`: income + deal + support points; supports=`is_primary_deal=false`) + endpoint `/api/reports/best-manager` (read-only, visibility-scoped via `topManagers` precedent, not `plans.manage`-gated) + `BestManagerReportRequest` (`year` required, `mode` in standard/absolute, `pipeline_id` exists) + `BestManagerResource` + 14 tests. FE `TabRating` (year-selector drives shared hub year via `setYear` — §1.1 single period source) + `LeaderCard` (hero) + `RatingTable` (medal top-3, out-of-standings muted). Standings = active non-service Manager/Director (spatie `role()` scope). See §6.6 as-built for supports/standings/FX definitions + the absolute-mode leader/rank gap.

**Ф4 — R4 Tasks + R5 Conversions. ✅ DONE (uncommitted, 2026-07-03, QA e2e PASS, 3924 PHPUnit).** `TaskMatrixService` (one count-matrix per task-like kind via `ActivityType::taskLikeValues()` + `ftm` sentinel routing through `Activity::scopeFtmCounted`; reuses `PlanTargetService::resolveScopeRows` — now `public` — so R4/R5 rows can never drift from the P-1 grid) + `ConversionReportService` (`custom` config-pairs + embedded honest `stage` chain) + standalone `StageConversionService` (`GET /api/reports/stage-conversions`, first-entry-only from `deal_stage_history`). Endpoints R4/R5 + `/stage-conversions`, thin controller, 3 FormRequests (`authorize():true`, visibility-scoped in the aggregator), 3 root-level (`$wrap=null`) Resources, factory states (`tasksCompleted`/`conversion` on PlanTargetFactory, `presentation`/`followUp` on ActivityFactory). 27 tests (11 R4 + 8 R5 + 8 stage). **Real driver-bug caught & fixed:** sqlite `strftime('%m')` zero-pads ("03") vs pgsql `EXTRACT(MONTH)` unpadded — a driver-aware `$monthParam` bind in `ConversionReportService::countPairSide` + `StageConversionService::firstEntryPerDealPerStage` (TaskMatrixService sidesteps it by grouping in PHP). Двойной прогноз — groundwork only, not wired. See §6.7/§6.8 as-built for the kind-axis, pairs-identity, and the stage-block shape delta.

> **As-built (Ф4, reviewer 2026-07-03):** the `general` auto-task-ratio block (§6.8 three-block promise) is **NOT shipped this pass** — only `custom` (config-authored pairs) + `stage` (honest chain) are returned. The FE `ConversionReportResponse.general` field is declared optional/forward-compat and degrades to `[]`. General auto-pairs are a Ф5+ follow-up (config-seeded defaults §4.4 exist but are not auto-materialised into report rows yet).

**Ф5 — R6 Product income + Excel export. ✅ DONE (uncommitted, 2026-07-04, QA e2e PASS, 3944 PHPUnit).** `ProductIncomeService` (interim won_deals fact by product line + open-deal `expected` by line, both attributed via `deal_products.product_id → catalog_products.group_id`; plan from `plan_targets(product_income, scope_type=company, scope_product_group_id)`; FX per §4.1; visibility-scoped over the underlying deals) + endpoint R6 (read-only, `authorize():true`, visibility-scoped in the aggregator) + `ProductIncomeReportRequest`/`ProductIncomeFilters`/`ProductIncomeResource` (root-level `$wrap=null`). Excel: `ReportXlsxExporter` (shape-specific `buildMatrixSheet`/`buildListSheet` on the `SalesDashboardService::buildXlsx` A1-notation pattern) + `ReportExportService` (one `export*` per report, re-running the SAME aggregator `build()`/`buildMatrix()` — no drift, no second query path) behind **7 export routes**: `GET /api/reports/{registry,income-schedule,best-manager,task-matrix,conversions,product-income}/export` + `GET /api/plans/matrix/export`, each streaming a .xlsx via a shared `streamXlsx` wrapper. 20 tests (11 R6 + 9 export incl. a visibility-scope byte-leak assertion). FE: `MetricProductIncome`/`useProductIncomeTab` (breakdown toggle product-line vs employee, both feeding `usePlanMatrixCore` via a pluggable `cellScope` stamp), plans + report Excel buttons on the hub shell (blob download via shared `@/utils/download`, RFC 5987 filename, `revokeObjectURL` cleanup). See as-built notes below.

> **As-built (Ф5, reviewer 2026-07-04):** two boundary/shape clarifications:
> - **R6 money-fact is a direct per-line-item aggregate, NOT a `ManagerKpiService` delegate.** §8.3 lists `ManagerKpiService` as the won-deal money-fact owner, but that service resolves income at **deal granularity per manager** — R6 needs money at **line-item granularity per product group** (`SUM(deal_products.amount)` grouped by `catalog_products.group_id`), which ManagerKpiService cannot supply. `ProductIncomeService` therefore runs its own batched read-join query (explicitly the §8.3 read-join allowance — «Sales owns deal_products; read-join for aggregates is allowed, same as `SalesDashboardService::topProducts`») and reuses `ManagerKpiService` only for `scorePct`/`scoreBadge` (the % + badge convention). Correct — no second money-fact abstraction, same effective-recognition-month expression as `SalesDashboardService::effectiveDateExpr`.
> - **Excel export never widens the read surface.** Every `ReportExportService::export*` re-runs the identical visibility-scoped aggregator the JSON sibling uses, so the .xlsx bytes carry exactly the rows the viewer could already see (verified by `test_product_income_export_is_visibility_scoped`). User strings (deal/manager/product names) are written as cell **values** via `setCellValue`, never as formulas — no CSV/formula-injection surface. Routes: each `{report}/export` is declared before its plain-JSON sibling (route-ordering consistency, harmless here since paths are unambiguous).
