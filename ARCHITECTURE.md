...
1. **Сначала ищи в том, что уже стоит.** Зависимости проекта = `composer.json` / `package.json` (копируем с Vizion) + то, что есть в `./examples/vizion/` и `./examples/contracts/`. Если задачу закрывает уже подключённая библиотека (Laravel/Spatie/Prism/PrimeVue/ECharts/PHPWord/vue-i18n/Sanctum и т.п.) — **используй её, ничего не ставь.**
