...
- **Эталон стека — `ARCHITECTURE.md` + `docs/backend-standard.md` + реальный `src/app/Domain/*`.** Очереди (`queue:work`, БЕЗ Horizon), scheduler (`routes/console.php`), jobs, async-флоу с lifecycle pending→running→done|error и idempotency через UNIQUE — паттерн бери в реальных `src/app/Domain/*/Jobs` (idempotent job: tries=1 + ShouldBeUnique). Архив `./examples/vizion/` — вторичная справка до cutover, стеком НЕ рулит.
...
2. **Технический паттерн** → реальные `src/app/Domain/*/Jobs` (idempotent job: tries=1 + ShouldBeUnique + lifecycle статусов), scheduler в `routes/console.php`. Архив `./examples/vizion/` — вторичная справка.
