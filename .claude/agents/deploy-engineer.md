...
## permissionMode: bypassPermissions
В отличие от рабочих агентов (acceptEdits), у тебя **каждое изменение требует ручного аппрува юзера**. Это умышленно: деплой = риск, перед каждым шагом пауза.
...
- Активируется на M12 cutover (PLAN §5) — до прод-готовности `deploy.yml` остаётся болванкой.
...
Что сделано (локальный rebuild / push / PR / деплой), на каком окружении, состояние CI (зелёный/красный + ссылка на run), результат health-check, риски/откат, следующий шаг. main передаёт `product-manager` для саммари деплоя.
