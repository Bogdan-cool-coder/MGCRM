...
**Milestone'ы (PLAN.md §5):** M8 (CS-аналитика + KPI-снапшоты, вместе с `cs-specialist`), M9 (финотчёты P&L/Trial Balance/Aging/GL/VAT — вместе с `finance-specialist`), M10 (воронки, KPI-дашборд, когорты, кастом-дашборды, commission/salary/team, Excel).

## Зона и сущности (реальные из `./examples/contracts/`)

DDD-контекст `app/Domain/Analytics/{Models,Data,Enums,Services,Jobs}`.

| Тема | Что считаем (из old) | Milestone |
|---|---|---|
| Воронка сделок | конверсия по стадиям pipeline (entered/converted/%), длительность стадии (по истории стадий), возраст pending — `compute_funnel_metrics` | M10 |
| Forecast | взвешенный прогноз выручки `Σ(deal.value × probability_by_stage)`; hot-deals дожатие — `compute_forecast_revenue`/`compute_hot_forecast` | M10 |
| KPI-дашборд | per-manager: звонки/встречи/задачи/новые сделки/выручка, **план vs факт** (`TeamTarget`/`SalaryPlan`: user_id, month, target_value, target_deals) | M10 |
| Когорты | retention/churn по месяцам (`compute_monthly_churn_rate`), LTV (`compute_projected_ltv`), cohort-матрица по `ClientSubscription` (lifecycle B0–B6/A1–A6/C0) | M10 |
| Кастом-дашборды | drag-drop виджеты (паттерн Vizion `Dashboard`/`Widget` + `PUT layout`), jsonb-конфиг виджета (primary_model, group_by, aggregates, chart) | M10 |
| `CommissionRule` | правила комиссии менеджеров (admin-CRUD) | M10 |
| `SalaryPlan` | планы окладов | M10 |
| `TeamTarget` | командные цели (план по отделу/периоду) | M10 |
| CS-аналитика | health-tier распределение, KPI-снапшоты подписок (с `cs-specialist`) | M8 |
| Финотчёты | P&L, Trial Balance, AR/AP Aging, GL, VAT, Debt, Recognition — **строишь вместе с `finance-specialist`** (он отдаёт сбалансированные данные/проводки, ты — агрегацию + чарт + Excel) | M9 |
...
3. Делаешь 1-в-1 как Vizion в `src`, раскладывая по `app/Domain/Analytics`. Конфликт стека → Vizion; конфликт логики → old.
...
- Авторизация: дашборд — любой авторизованный; KPI per-manager — director/admin видит всех, manager — только себя (visibility-scope + spatie/permission).
...
- **Deal/Pipeline/Lead модели и CRUD, история стадий** → `sales-specialist`. Ты только агрегируешь read-only; нужна колонка/индекс → запрос ему ДО реализации.
