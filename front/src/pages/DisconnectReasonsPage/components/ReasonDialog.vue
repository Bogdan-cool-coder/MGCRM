<template>
  <Dialog
    ...
    :header="editing ? t('admin.disconnectReasons.edit') : t('admin.disconnectReasons.add')"
    modal
    :style="{ width: '26rem' }"
    :draggable="false"
  >
    <div class="row g-3">
      <!-- Name -->
      <div class="col-12">
        <label class="dir-dialog__label">{{ t('admin.disconnectReasons.fields.name') }}</label>
        <InputText
          v-model="form.name"
          class="w-100 mt-1"
      ...
      <!-- Sort order -->
      <div class="col-12">
        <label class="dir-dialog__label">{{ t('admin.disconnectReasons.fields.sortOrder') }}</label>
      ...
      <div class="col-12 d-flex align-items-center gap-2">
        ...
        <label class="mb-0 dir-dialog__label">{{ t('admin.disconnectReasons.fields.isActive') }}</label>
      </div>
    </div>
...
import type { ReasonFormPayload } from '../composables/useDisconnectReasonsPage'

const props = defineProps<{
  modelValue: boolean
  editing: DisconnectReason | null
  loading: boolean
}>()

const emit = defineEmits<{
  'update:modelValue': [value: boolean]
  save: [payload: ReasonFormPayload]
}>()
...
const form = ref<ReasonFormPayload>({
  name: '',
  sort_order: 0,
  is_active: true,
})
