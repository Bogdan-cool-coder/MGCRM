...
**«Планы и отчёты» (Sales Analytics) — Ф1 DONE (uncommitted, 2026-07-03, 3860 PHPUnit).** Универсальная ячейка плана `plan_targets` (metric×scope×period×layer; surrogate `scope_key`+`period_month_key` несут DB-UNIQUE поверх nullable-скоупов) + `plan_target_audits` (append-only, DealAudit-паттерн, FK `nullOnDelete`). `new_income scope=user` end-to-end: matrix-read (P-1) / bulk-upsert+selective-audit (P-2) / copy-previous (P-3); `plans.manage`→admin+director, reads visibility-scoped (не gated, §8.2). `PlanTargetService` реюзит `ManagerKpiService::scorePct/scoreBadge` + won-deal+FX; enums `PlanMetric/PlanScopeType/PlanLayer`. FE: DashboardPage→analytics-hub (табы+keep-alive+`?tab=`), Обзор перенесён без изменений в `TabOverview`; `TabPlans`+`PlanMatrix` inline-грид. Контракт-канон `docs/contracts/plan-targets-api-contract.md` (Ф0); бизнес-логика — `docs/audit/SpaceCRM-reports-analysis-2026-07.md`. R1–R6 (реестр/дожим/график НП/лучший менеджер/задачи/конверсии/линейки) + Excel-экспорт — Ф2–Ф5; МК-линк — Ф6; payment-fact — Ф7. **Known-gap Ф1:** план-сторона matrix не конвертирует валюту ячейки в base (RUB-first; чинится при мультивалютных планах Ф2+).
