...
| **M5/N6** Генерация договоров (PHPWord→Gotenberg→PDF) | Документы | **QA-fail (P1, частично)** | **Генерация разблокирована на dev/staging** (`TemplateVersionSeeder` + committed seed.docx); `current_version_id` теперь заполняется при `db:seed`. Live (прод): нужен `php artisan db:seed --class=TemplateVersionSeeder` + `php artisan db:seed --class=ApprovalRouteSeeder`. Approval-summary API исправлен. **Открыто:** IDOR DOC-2; нет UI лицензиаров; реальные docx-шаблоны (юр-ревью); attempt double-increment. |
