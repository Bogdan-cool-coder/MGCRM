...
- [x] **Уведомления in-app (task #9, PM APPROVE pending commit, 2026-06-17):** `Notification` domain (`app/Domain/Notification/`): модель, `NotificationCategory` enum (task/approval/deal/mention/system), `NotificationService` (createForUser/dispatch/grouped/markRead/markAllRead/unreadCount/digest), `NotificationPolicy` (строго recipient-scoped), `NotificationController` (GET /api/notifications, POST read/{id}, POST read-all), `NotificationFeedResource`+`NotificationResource`, `NotifyActivityAssigneeListener` (ActivityAssigned → actionable task-нотификация), `NotifyApproversListener` (DocumentSubmittedForApproval → actionable approval-нотификация). Миграция-патч `2026_06_22_100000_add_action_label_to_notifications_table` — отдельная (applied dev Postgres). Фронт: `notificationsStore`, `useNotificationsFlyout`, `NotificationsButton.vue` flyout в Орбите (actionable-bucket / feed / digest-chips / loadMore / mark-on-close). 25 тестов (13 feature API + 5 unit dispatch + 7 unit ApprovalNotificationService). **Фиксы по итогам ревью:** (1) `feed_page` — `->paginate($perPage, pageName: 'feed_page')` в сервисе + фронт `params: { feed_page: N }` — load-more работает, тест `test_feed_second_page_returns_next_records_not_duplicates_of_first` PASS; (2) `NotificationDto` entity выровнен: поля `category/is_actionable/action_label/payload/is_read/read_at` — соответствуют контракту. Беклог (не блокер): `fetchUnreadCount()` в store делает полный grouped-запрос — over-fetch; отдельный легковесный endpoint (`GET /api/notifications/count`) — backlog M6.
