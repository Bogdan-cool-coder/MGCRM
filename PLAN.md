...
**M6. Активности / Задачи + Уведомления (Продажи)** — `Activity`+`Notification`. Задачная половина зрелая; in-app уведомления done-merged. TODO: Activity (call/meeting/task/note)+таймлайн; **починить отчёт о встрече+FTM** (report не штампует engagement/log, FTM не пишется), inline status→done маршрутизировать в `complete`; **колокольчик в sidebar-режим** (сейчас только «Орбита»), Telegram-link `deeplink` (FE читал `link_url`), Preferences/шаблоны/broadcast.
