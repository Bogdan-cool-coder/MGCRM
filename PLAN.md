...
**«Планы и отчёты» (Sales Analytics) — Ф1+Ф2 DONE (uncommitted, 2026-07-03, 3882 PHPUnit, QA e2e PASS).** Универсальная ячейка плана `plan_targets` (metric×scope×period×layer; surrogate `scope_key`+`period_month_key` несут DB-UNIQUE поверх nullable-скоупов) + `plan_target_audits` (append-only, DealAudit-паттерн, FK `nullOnDelete`). `new_income scope=user` end-to-end: matrix-read (P-1) / bulk-upsert+selective-audit (P-2) / copy-previous (P-3); `plans.manage`→admin+director, reads visibility-scoped (не gated, §8.2). `PlanTargetService` реюзит `ManagerKpiService::scorePct/scoreBadge` + won-deal+FX; enums `PlanMetric/PlanScopeType/PlanLayer`. FE: DashboardPage→analytics-hub (табы+keep-alive+`?tab=`), Обзор перенесён без изменений в `TabOverview`; `TabPlans`+`PlanMatrix` inline-грид. **Ф2:** R1 «Реестр+Дожим» (`ExpectedIncomeRegistryService`: expected/squeeze/no_date-списки) + R2 «График НП» (`IncomeScheduleService`: день-календарь + кумулятивные план/факт, распределение по рабочим дням, остаток в последний день) — read-only endpoints `/api/reports/{registry,income-schedule}`, visibility-scoped, не gated; FE `TabRegistry/RegistryTable` + `TabSchedule/NpCalendar/ScheduleLegend`+ECharts; rollup плана делит `PlanTargetService::scopedUserIds` с matrix (без drift); план идентичен при любом pipeline-фильтре (user-ячейка без воронка-измерения в Ф1). Контракт-канон `docs/contracts/plan-targets-api-contract.md` (§6.4/§6.5 as-built). R3–R6 (лучший менеджер/задачи/конверсии/линейки) + Excel-экспорт — Ф3–Ф5; МК-линк — Ф6; payment-fact — Ф7. **Ф1 known-gap ЗАКРЫТ (Ф2):** план-сторона matrix конвертирует валюту ячейки в base симметрично факту (`PlanMatrixPlanFxTest`).
