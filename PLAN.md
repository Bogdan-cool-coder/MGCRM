...
| **M7** Автоматизации + node-полотно | сквозной | done-merged | **движок built, never run в проде** (0 правил); ретеншн-прун ломает идемпотентность (#48); **волна 4:** CreateTaskConfig/TgNotifyConfig FE прошиты на канонические ключи движка (body/responsible spec/recipient spec); ValidatesAutomationConfig валидирует правильные поля — chosen assignee/recipient теперь реально применяются при запуске |
