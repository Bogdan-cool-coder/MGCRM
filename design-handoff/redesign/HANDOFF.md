...
### Обновление 2026-07-04 вечер — трек консистентности В1–В4 ИСПОЛНЕН
Все 4 волны плана `docs/audit/Frontend-Consistency-Audit-2026-07.md` §5 исполнены локально
(6 коммитов `4610826`→`e7d4b64`, незакоммичено в main до отдельного шага): В1 «сломанное молча»
(фантом-Bootstrap + мёртвые PrimeVue-3 токены канваса + битые иконки + единая фильтр-строка Обзора +
EntityAvatar на задачах), В2 dark-долг (~119 точек / 27 файлов, −202 строки), В3 периферия
(shared `ListToolbar`/`SegmentedControl`/`FilterTriggerButton`/`PaginatorCaption`, каталог/документы/
онбординг/автоматизации на канон-шасси), В4 швы (5 самодельных оверлеев DealPage → house-примитивы
−350 строк, money 2 канон-формы, NotFoundState ×6, пагинации, confirm-лейблы) + бонус имён вместо
FK-ID в лентах. Итог + честный счёт закрытого/оставшегося — **audit §5 (статусы) + §7 «Итог
исполнения»**. Осознанно оставлено: `RequisiteCard` grouped-confirm, crm/entity muted-хвост (H24),
`:draggable="false"` по диалогам, CrmFeed log-виджет-имена. Сьют 4081→4097/0, все гейты 0, ~7 QA-гейтов.
